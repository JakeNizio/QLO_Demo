...
#Trial 5 - make each route its own csv, limit to 5 routes
# for i, (route_id, route_data) in enumerate(json_data.items()):
#     if i >= 5:  # Stop after 5 routes
#         break
...
#Trial 6 - writing gps location to file as well
# Process each route
for route_id, route_info in json_data.items():
    csv_file = f"{route_id}.csv"  # Name CSV file after the route
    ...
    with open(csv_file, mode='w', newline='') as file:
        ...
        # Write the header row
        writer.writerow(["Stop", "Latitude", "Longitude"])

        # Write stop data
        for stop_id, stop_info in route_info["stops"].items():
            writer.writerow([stop_id, stop_info["lat"], stop_info["lng"]])